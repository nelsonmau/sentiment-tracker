...
class EmbedRequest(webapp.RequestHandler):
    @helpers.write_response
    def get(self, pollname):
        poll = models.Poll.get_by_name(pollname)
        path = os.path.join(os.path.dirname(__file__), "templates", "embed.html")
        return template.render(path, {'poll': poll})

class RegisterIncrRequest(webapp.RequestHandler):
    @helpers.write_response
    def get(self, pollname, choice):
